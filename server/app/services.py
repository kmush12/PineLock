...
async def sync_device(device_id: str):
    ...
    try:
        async with async_session_maker() as session:
            ...
            # - Master cards (card_type == 'master')
            # - Key tags are handled separately or as part of the list with a flag?
            #   The user wants "Master Card opens everything", "Key Tag checks presence".
            #   So the device needs to know WHICH UID is the Key Tag.
            ...
            for card in all_cards:
                if card.card_type == 'key_tag':
                    # This should be the key tag for THIS lock
                    if card.lock_id == lock.id:
